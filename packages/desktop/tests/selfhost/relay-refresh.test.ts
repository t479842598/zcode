...
test("duplicate identity does not restart the outgoing sequence", (t) => {
  ...
  relay.setFrameIdentity({ ...oldIdentity });
  relay.socket.write(writer.buffer);
  assert.deepEqual(sent.map((frame) => frame.payload.seq), [1, 2]);
});
...
test("refresh releases the old agent scope before creating a fresh client identity", async (t) => {
  ...
  class ObservedServices extends ServiceCollection {
    override exposeOnChannelServer(server: any, overrides: ReadonlyMap<string, unknown> = new Map()) {
  ...
  t.after(async () => { await lifecycle.dispose(); relay.socket.dispose(); });
  ...
  await scopes[0].initializeConversationV4({ kind: "clientHello", protocolVersion: hello.protocolVersion, clientId: "client-old", appVersion: "fixture" });
  ...
  await scopes[1].initializeConversationV4({ kind: "clientHello", protocolVersion: hello.protocolVersion, clientId: "client-new", appVersion: "fixture" });
  await lifecycle.prepare({ ...newIdentity });
...
test("desktop scope remains continuous while relay remains replayable", async () => {
  ...
  const desktop = createZCodeAgentConnectionScope({} as any, { connectionId: "desktop-fixture", clientMode: "desktop-continuous", role: "terminal-client" });
